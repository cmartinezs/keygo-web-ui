...
|---|---|---|---|---|
| Recuperacion de cuenta publica | POST /api/v1/tenants/keygo/account/forgot-password | No encontrado | POST /api/v1/tenants/{tenantSlug}/users/{userId}/reset-password | No equivalen. El existente es reset administrativo (tenant-admin), no forgot/reset self-service de usuario final |
| Reset por codigo de recuperacion | POST /api/v1/tenants/keygo/account/reset-password | No encontrado | POST /api/v1/tenants/{tenantSlug}/users/{userId}/reset-password | No equivalen por flujo, auth y actor |
...
2. docs/FUNCTIONAL_GUIDE.md seccion 5 mezcla items ya implementados con items realmente pendientes.
...
2. Cerrar definicion e implementacion de flujo forgot/reset password (endpoint + UI publica).
