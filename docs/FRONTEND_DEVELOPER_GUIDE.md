 ...
 *
 * IMPORTANTE — Sensibilidad al ambiente del backend:
 *  El contenido de `clientMessage` varía según el perfil de Spring Boot activo:
 *  - Perfiles `dev` / `local`: puede incluir texto técnico (nombre de excepción, causa)
 *    útil para diagnóstico en desarrollo.
 *  - Perfil `prod` / `staging`: siempre amigable para el usuario final.
 *  El frontend debe mostrar `data.clientMessage` directamente sin reescribirlo.
 *  El backend ya realizó la adaptación al ambiente.
 *
 * Guia de uso segun `origin`:
...
export interface ErrorData {
   ...
   * Específico por tipo de excepción. En dev/local puede ser técnico;
   * en prod/staging siempre es amigable para el usuario final.
   * Mostrar directamente, sin sustituir ni reescribir en el cliente.
   */
...
/**
 * Extrae el mensaje listo para mostrar al usuario.
 * El backend adapta `clientMessage` al ambiente (dev/local = técnico, prod = amigable).
 * No reescribir ni sustituir este valor — mostrar directamente.
 */
...
- **Mostrar siempre `data.clientMessage` directamente.** El backend adapta el contenido al perfil activo: en `dev`/`local` puede ser técnico; en `prod` es siempre amigable para el usuario final. No reescribir ni mapear manualmente mensajes desde el código de error.
- Si `data.origin=CLIENT_REQUEST` y `data.clientRequestCause=USER_INPUT`, el problema es de datos capturados por el usuario (ej. credenciales inválidas). Mostrar `clientMessage` en el formulario.
- Si `data.origin=CLIENT_REQUEST` y `data.clientRequestCause=CLIENT_TECHNICAL`, el problema es de integración de la UI (ej. cookie de sesión, parámetros OAuth). Loguear para el equipo de desarrollo; no mostrar como error del usuario.
- Si `data.origin=BUSINESS_RULE`, la solicitud fue técnicamente correcta pero bloqueada por reglas del dominio (ej. email no verificado, membership inactivo). Mostrar `clientMessage` y ofrecer acción alternativa si aplica.
- Si `data.origin=SERVER_PROCESSING`, mostrar `clientMessage` (que ya incluye instrucción de reintento) y loguear en monitoreo.

| Error en login | `origin` | `clientRequestCause` | Acción en UI |
|---|---|---|---|
| Credenciales incorrectas | `CLIENT_REQUEST` | `USER_INPUT` | Mostrar `clientMessage` en formulario |
| Sesión expirada / sin JSESSIONID | `CLIENT_REQUEST` | `USER_INPUT`* | Mostrar `clientMessage`, reiniciar flujo |
| Membership inactivo | `BUSINESS_RULE` | — | Mostrar `clientMessage`, ofrecer contactar admin |
| Email no verificado | `BUSINESS_RULE` | — | Mostrar `clientMessage`, ofrecer reenvío de código |
| Error interno del servidor | `SERVER_PROCESSING` | — | Mostrar `clientMessage`, loguear |
...
> **Regla sobre `clientMessage`:** el backend adapta el contenido al perfil activo. En `dev`/`local` puede incluir texto técnico para diagnóstico; en `prod`/`staging` siempre es amigable para el usuario final. El frontend nunca debe construcciones propias de mensajes a partir del `code` o la excepción — mostrar `data.clientMessage` directamente.
