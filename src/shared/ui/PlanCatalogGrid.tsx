  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center gap-4 py-12 text-center" role="alert">
        ...
        <div>
          <p className="text-slate-600 font-medium">{t('subscribe.catalog.errorTitle')}</p>
          <p className="text-sm text-slate-400 mt-1">{t('subscribe.catalog.errorDescription')}</p>
        </div>
        {onRetry && (
          <button
            type="button"
