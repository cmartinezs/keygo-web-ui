...
function createSchema() {
  return z.object({
    name: z
      .string()
      .min(1, i18n.t('adminTenantCreate.nameRequired'))
      .max(255, i18n.t('adminTenantCreate.nameMax')),
    owner_email: z
      .string()
      .min(1, i18n.t('adminTenantCreate.ownerEmailRequired'))
      .email(i18n.t('adminTenantCreate.ownerEmailInvalid')),
  })
}
...
export default function TenantCreatePage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(t('adminTenantCreate.createdSuccess', { name: data.name }))
    ...
    <div className="p-6 max-w-lg space-y-6">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          aria-label={t('adminTenantCreate.backAria')}
        >
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('adminTenantCreate.title')}</h2>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
            {t('adminTenantCreate.subtitle')}
          </p>
        </div>
      </div>
      ...
      >
        <Field
          label={t('adminTenantCreate.orgNameLabel')}
          hint={t('adminTenantCreate.orgNameHint')}
          error={errors.name?.message}
        >
          <input
            type="text"
            placeholder={t('adminTenantCreate.orgNamePlaceholder')}
            autoFocus
            {...register('name')}
        ...
        <Field
          label={t('adminTenantCreate.ownerEmailLabel')}
          hint={t('adminTenantCreate.ownerEmailHint')}
          error={errors.owner_email?.message}
        >
          <input
            type="email"
            placeholder={t('adminTenantCreate.ownerEmailPlaceholder')}
        ...
        <div className="flex items-center justify-end gap-3 pt-1">
          ...
          >
            ...
            {isBusy ? t('adminTenantCreate.creating') : t('adminTenantCreate.create')}
          </button>
        </div>
      </form>
    </div>
