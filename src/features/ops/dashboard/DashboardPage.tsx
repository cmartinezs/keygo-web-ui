...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="max-w-screen-xl mx-auto space-y-8">
      ...
      {!isLoading && (data?.quick_actions?.length ?? 0) > 0 && (
        <nav
          aria-label={t('adminDashboard.quickActions')}
          className="flex flex-wrap items-center gap-2"
        >
          <span className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white">
          ...
          {data?.quick_actions.map((action) => (
            <Link
              ...
              to={action.route}
              className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-100 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5"
            >
              {action.label}
            </Link>
          ))}
      ...
      <section aria-labelledby="section-service">
        <SectionTitle>
          <span id="section-service">{t('adminDashboard.sections.serviceStatus')}</span>
        </SectionTitle>
        {isLoading ? (
          ...
          <ServiceStatusRow
            ...
            activeSigningKey={data?.security.active_signing_key}
          />
        )}
      </section>
      ...
      <section aria-labelledby="section-iam">
        <SectionTitle>
          <span id="section-iam">{t('adminDashboard.sections.iamCore')}</span>
        </SectionTitle>
        {isLoading ? (
          <SkeletonGrid count={4} />
        ) : (
      ...
      <section aria-labelledby="section-security">
        <SectionTitle>
          <span id="section-security">{t('adminDashboard.sections.security')}</span>
        </SectionTitle>
      ...
      <section aria-labelledby="section-lists">
        <SectionTitle>
          <span id="section-lists">{t('adminDashboard.sections.managementAndActivity')}</span>
        </SectionTitle>
        {isLoading ? (
      ...
      <section aria-labelledby="section-rankings">
        <SectionTitle>
          <span id="section-rankings">{t('adminDashboard.sections.rankings')}</span>
        </SectionTitle>
        {isLoading ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <CardSkeleton />
      ...
      <section aria-labelledby="section-onboarding">
        <SectionTitle>
          <span id="section-onboarding">{t('adminDashboard.sections.onboardingHealth')}</span>
        </SectionTitle>
        {isLoading ? (
          <SkeletonGrid count={4} />
        ) : (
          <OnboardingHealthRow registration={data?.registration} />
        )}
      </section>
    </div>
  );
